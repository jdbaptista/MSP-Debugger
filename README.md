## Introduction
This project is a combination of firmware and hardware that displays the assembly instructions stored in the flash memory of an MSP430G2553, specifically the 20 pin DIP package. The device is shown below and supports the following button interactions:
...
0xC008: BIS.B #1 &0xE3D2
0xC00C: XOR.B #1 &0x40B1
```
...
```
0xC01A: JEQ 0xC00C <
0xC01C: SUB.B #1 0x9381(SP)
0xC020: CMP.B #0 0x27F3(SP)
0xC024: JEQ 0xC00C
```
The first jump instruction that is found is at `0xC01A`, jumping to `0xC00C`. This isn't useful in itself because we don't know what was being compared, which was an instruction that was skipped over.

Using the **UP** button moves the cursor backward by one instruction.
```
0xC016: CMP.B #0 0x27F8(SP) <
0xC01A: JEQ 0xC00C
0xC01C: SUB.B #1 0x9381(SP)
0xC020: CMP.B #0 0x27F3(SP)
```
Moving the cursor down to the `JEQ 0xC00C` and using the **FOLLOW JMP** instruction brings the cursor to `0xC00C`.
```
0xC00C: XOR.B #1 &0x40B1 <
0xC010: MOV.B #0x2710 0x0000(SP)
0xC016: CMP.B #0 0x27F8(SP)
